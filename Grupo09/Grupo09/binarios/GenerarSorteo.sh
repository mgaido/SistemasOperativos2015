...
function Fecha  {
	...
	for (( linea=1; linea<=$Lineas; linea=$[$linea+1] )); do
		fechaArch=`head -$linea $MAEDIR"/FechasAdj.csv" | tail -1 | sed 's/^\([^;]*\).*/\1/g' | sed 's/\//-/g' | sed 's/^\([0-9]*\)-\([0-9]*\)-\([0-9]*\)/\3\2\1/g'`
		if [ $fechaArch -ge $fecha ]; then
			if [ $fechaArch -le $Prox ]; then
				Prox=$fechaArch
			fi
		fi

	done
	nombreArch=`echo $Prox | sed 's/^\([0-9][0-9][0-9][0-9]\)\([0-9][0-9]\)\([0-9][0-9]\)/\3-\2-\1/g'`
}
